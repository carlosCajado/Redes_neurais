...
dados = readtable('car_treino.csv');
...
save('pesos_bias_treinados_relu.mat', 'Whi', 'Woh', 'bias_hi', 'bias_oh');
...
function resultado = ReLu(vetor)
    % Inicializa um vetor de zeros com o mesmo tamanho do vetor de entrada
    resultado = zeros(size(vetor)); 
    % Aplica a função ReLU a cada elemento do vetor
    for i = 1:numel(vetor)
        resultado(i) = max(0, vetor(i)); % Mantém o valor se for positivo, caso contrário, atribui zero
    end
end